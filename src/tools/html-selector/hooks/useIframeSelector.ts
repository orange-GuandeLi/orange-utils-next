...
export function getElementSelector(el: HTMLElement): string {
  ...
  while (current && current.nodeType === Node.ELEMENT_NODE) {
    ...
    if (current.parentElement) {
      const sameTag = Array.prototype.slice
        .call(current.parentElement.children)
...
export function getSelectionInfo(el: HTMLElement, iframe: HTMLIFrameElement): SelectionInfo {
  ...
  if (editableType === "image") {
    ...
    editableValue = (el as HTMLAnchorElement).href || el.getAttribute("href") || null
  ...
  return {
    ...
    rect: {
      ...
      top: Math.round(rect.top + (iframe.contentDocument?.documentElement.scrollTop || 0)),
      left: Math.round(rect.left + (iframe.contentDocument?.documentElement.scrollLeft || 0)),
    },
    editableType,
    editableValue,
...
function getElementsAtPoint(doc: Document, x: number, y: number): HTMLElement[] {
...
export function useIframeSelector({
  ...
  // 这是 useRef 镜像的标准模式：3 个 ref 互调后用 useEffect 同步最新版本
  ...
  const renderBreadcrumbRef = useRef<
    (breadcrumb: HTMLDivElement, domPath: HTMLElement[], iframe: HTMLIFrameElement) => void
  >(() => {})

  // 渲染面包屑
  const renderBreadcrumb = useCallback(
    (breadcrumb: HTMLDivElement, domPath: HTMLElement[], iframe: HTMLIFrameElement) => {
  ...
  // eslint-disable-next-line react-hooks/refs -- ref 镜像，打破循环依赖
  renderBreadcrumbRef.current = renderBreadcrumb

  // 更新高亮位置
  const updateHighlight = useCallback((el: HTMLElement, iframe: HTMLIFrameElement) => {
        ...
        if (e.key === "Tab") {
          ...
          if (overlapping.length > 1) {
            overlappingIdxRef.current =
              (overlappingIdxRef.current + (e.shiftKey ? -1 : 1) + overlapping.length) %
              overlapping.length
